...
const getSegmentationColor = (classId) => {
  // Bright, distinct colors for tumor regions
  const colors = {
    1: { r: 255, g: 50, b: 50 },    // NCR (Necrotic Core) - Bright Red
    2: { r: 50, g: 255, b: 50 },    // ED (Edema) - Bright Green  
    3: { r: 50, g: 150, b: 255 },   // ET (Enhancing Tumor) - Bright Blue
  };
  return colors[classId] || { r: 255, g: 255, b: 255 };
};
...
const SliceViewer = ({ data, slice, axis, colorMap, overlay, overlayOpacity, title }) => {
  ...
  useEffect(() => {
    ...
    if (overlay && overlay.values) {
      let overlaySlice;
      if (axis === 'axial') {
        overlaySlice = getAxialSlice(overlay, slice);
      } else if (axis === 'sagittal') {
        overlaySlice = getSagittalSlice(overlay, slice);
      } else {
        overlaySlice = getCoronalSlice(overlay, slice);
      }
      
      if (overlaySlice && overlaySlice.length > 0) {
        const overlayImageData = ctx.getImageData(0, 0, width, height);
        
        for (let i = 0; i < overlaySlice.length; i++) {
          const segClass = Math.round(overlaySlice[i]);
          if (segClass > 0 && segClass <= 3) {
            const idx = i * 4;
            const color = getSegmentationColor(segClass);
            // Blend overlay with MRI
            overlayImageData.data[idx] = Math.round(color.r * overlayOpacity + overlayImageData.data[idx] * (1 - overlayOpacity));
            overlayImageData.data[idx + 1] = Math.round(color.g * overlayOpacity + overlayImageData.data[idx + 1] * (1 - overlayOpacity));
            overlayImageData.data[idx + 2] = Math.round(color.b * overlayOpacity + overlayImageData.data[idx + 2] * (1 - overlayOpacity));
          }
        }
...
const Visualization3D = ({ mriData, segmentationData, insights, hasProcessedData = false }) => {
  ...
  const fetchRealData = useCallback(async () => {
    ...
    try {
      ...
      const [mriResponse, segResponse] = await Promise.all([
        getMRIVolume(selectedModality, 2), // Downsample by 2 for performance
        getSegmentationVolume(2)
      ]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm mt-0.5 flex items-center gap-2">
              Interactive brain volume exploration
              {isUsingRealData ? (
          ...
          >
            {isLoading ? (
              ...
              <>
                ...
                {isUsingRealData ? 'Refresh Data' : 'Load Real Data'}
              </>
            )}
          </motion.button>
      ...
      {/* Controls Bar */}
      <div className="flex flex-wrap items-center gap-4 p-4 bg-gray-800/30 rounded-2xl border border-gray-700/50">
        {/* Modality Selector */}
        ...
        >
          ...
          <span className="text-xs">Segmentation</span>
        </button>
        ...
        {showOverlay && (
          <div className="flex items-center gap-2">
            ...
            <input
              ...
              className="w-20 accent-ne3na3-primary"
            />
      ...
      <AnimatePresence mode="wait">
        {activeView === 'orthogonal' && (
          ...
          >
            {/* Axial View */}
            <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-white">Axial (Top-Down)</span>
                <span className="text-xs text-gray-500">Slice: {axialSlice}/{maxSlices.axial}</span>
              </div>
              ...
              <input
                ...
                className="w-full accent-ne3na3-primary"
              />
            </div>
            ...
            <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-white">Sagittal (Side)</span>
                <span className="text-xs text-gray-500">Slice: {sagittalSlice}/{maxSlices.sagittal}</span>
              </div>
              <div className="aspect-square bg-gray-900 rounded-xl overflow-hidden">
                <SliceViewer
                  data={currentData}
            ...
            <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium text-white">Coronal (Front)</span>
                <span className="text-xs text-gray-500">Slice: {coronalSlice}/{maxSlices.coronal}</span>
              </div>
              <div className="aspect-square bg-gray-900 rounded-xl overflow-hidden">
                <SliceViewer
                  data={currentData}
            ...
            <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4">
              <h4 className="text-sm font-medium text-white mb-4">Segmentation Legend</h4>
              <div className="space-y-3">
                {[
                  { label: 'NCR (Necrotic Core)', color: 'bg-red-500', desc: 'Dead tumor tissue' },
                  { label: 'ED (Edema)', color: 'bg-green-500', desc: 'Swelling around tumor' },
                  { label: 'ET (Enhancing Tumor)', color: 'bg-blue-500', desc: 'Active tumor region' },
                ].map((item) => (
              ...
              <div className="mt-6 pt-4 border-t border-gray-700/50">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <span className="text-gray-400"><kbd className="px-1 bg-gray-700 rounded">Space</kbd> Play/Pause</span>
                  <span className="text-gray-400"><kbd className="px-1 bg-gray-700 rounded">↑↓</kbd> Navigate</span>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {activeView === '3d' && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4"
          >
            <div className="aspect-video min-h-[400px] rounded-xl overflow-hidden">
              <VolumeRenderer3D />
            </div>
            <p className="text-xs text-gray-500 text-center mt-3">
              Drag to rotate • Scroll to zoom • Click Reset to restore view
            </p>
          </motion.div>
        )}
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-gray-800/30 rounded-2xl border border-gray-700/50 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-white">Original MRI</span>
                </div>
                <div className="aspect-square bg-gray-900 rounded-xl overflow-hidden">
                  <SliceViewer
                    data={currentData}
              ...
              <div className="bg-gray-800/30 rounded-2xl border border-ne3na3-primary/30 p-4 space-y-3">
                <div className="flex items-center gap-2">
                  <Activity className="w-5 h-5 text-ne3na3-neon" />
                  <span className="text-sm font-medium text-white">With Segmentation</span>
                </div>
                <div className="aspect-square bg-gray-900 rounded-xl overflow-hidden">
                  <SliceViewer
                    data={currentData}
            ...
            <div className="bg-gray-800/30 rounded-xl p-4 border border-gray-700/50">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm text-gray-400">Navigate Slices</span>
                <span className="text-xs text-gray-500">Slice {axialSlice} of {maxSlices.axial}</span>
              </div>
              <input
                type="range"
                min="0"
      ...
      <motion.div 
        className="flex items-start gap-4 p-5 bg-gradient-to-br from-ne3na3-primary/10 to-ne3na3-dark/5 
                   border border-ne3na3-primary/20 rounded-2xl"
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        ...
        <div className="text-sm">
          <p className="text-ne3na3-neon font-semibold mb-1">Interactive Visualization</p>
          <p className="text-gray-400 leading-relaxed">
            Explore brain MRI data in multiple views. Use the slider to navigate through slices,
            toggle segmentation overlay to see detected tumor regions, and switch between 
            orthogonal, 3D, and comparison views.
          </p>
        </div>
      </motion.div>
    </div>
